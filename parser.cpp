...
public:
    LetExpression(const std::string& variable, std::shared_ptr<Expression> value, std::shared_ptr<Expression> body)
...
public:
    IfExpression(const std::shared_ptr<Expression> cnd, std::shared_ptr<Expression> thn, std::shared_ptr<Expression> els)
...
private:
        ...
        for (char ch : program) {
            switch (ch) {
                ...
                case ' ':
                    ...
                    if (ch != ' ') tokens.push_back(std::string(1, ch));
                    break;
                default:
                    currentToken += ch;
                    break;
    ...
    static std::shared_ptr<Expression> parseExpression(const std::vector<std::string>& tokens, size_t& index) {
