  ...
  anf_to_select(const std::shared_ptr<Expression> &anf, size_t &counter,
    ...
    if (std::shared_ptr<LetExpression> anf_let =
      ...
      if (stack.find(let_var) == stack.end()) {
        ...
        if (std::shared_ptr<NumberExpression> number_expr =
                std::dynamic_pointer_cast<NumberExpression>(value)) {
        ...
        } else if (std::shared_ptr<LessExpression> less_expr =
                       std::dynamic_pointer_cast<LessExpression>(value)) {
          std::shared_ptr<Expression> left = less_expr->get_left();
          std::shared_ptr<VariableExpression> var_expr =
              std::dynamic_pointer_cast<VariableExpression>(left);
          std::shared_ptr<Expression> right = less_expr->get_right();
          std::shared_ptr<NumberExpression> num_expr =
              std::dynamic_pointer_cast<NumberExpression>(right);
          ...
          if (stack.find(var) != stack.end()) {
            ...
            std::vector<std::string> cmp = {"cmpq", immediate_,
                                            stack_location_};
    ...
    } else if (std::shared_ptr<IfExpression> if_expr =
                   std::dynamic_pointer_cast<IfExpression>(anf)) {
      std::shared_ptr<Expression> cnd = if_expr->get_cnd();
      std::shared_ptr<VariableExpression> var_cnd =
          std::dynamic_pointer_cast<VariableExpression>(cnd);
    ...
    } else if (std::shared_ptr<NumberExpression> num_expr =
                   std::dynamic_pointer_cast<NumberExpression>(anf)) {
