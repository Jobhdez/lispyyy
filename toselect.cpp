...
private:
  ...
  anf_to_select(const shared_ptr<Expression> &anf, size_t &counter,
    ...
    if (shared_ptr<LetExpression> anf_let =
    ...
    } else if (shared_ptr<SetExpression> set_expr =
                   dynamic_pointer_cast<SetExpression>(anf)) {
      string var = set_expr->get_variable();
      shared_ptr<Expression> exp = set_expr->get_value();
      if (shared_ptr<NumberExpression> num_expr =
              dynamic_pointer_cast<NumberExpression>(exp)) {
      ...
      } else if (shared_ptr<AdditionExpression> sum =
                     dynamic_pointer_cast<AdditionExpression>(exp)) {
        shared_ptr<Expression> left_exp = sum->get_left();
        shared_ptr<Expression> right_exp = sum->get_right();

        if (shared_ptr<VariableExpression> l_var =
                dynamic_pointer_cast<VariableExpression>(left_exp)) {
          if (shared_ptr<NumberExpression> r_num =
                  dynamic_pointer_cast<NumberExpression>(right_exp)) {
            vector<string> mov_ = {"movq", to_string(r_num->get_value()),
                                   "%rax"};
          ...
          } else if (shared_ptr<VariableExpression> r_var =
                         dynamic_pointer_cast<VariableExpression>(right_exp)) {
    ...
    } else if (shared_ptr<BeginExpression> bgn_exps =
                   dynamic_pointer_cast<BeginExpression>(anf)) {
    ...
    } else if (shared_ptr<WhileExpression> while_expr =
                   dynamic_pointer_cast<WhileExpression>(anf)) {
      shared_ptr<Expression> cnd = while_expr->get_cnd();
      if (shared_ptr<LessExpression> cnd_expr =
              dynamic_pointer_cast<LessExpression>(cnd)) {
        shared_ptr<Expression> left = cnd_expr->get_left();
        shared_ptr<VariableExpression> var_e =
            dynamic_pointer_cast<VariableExpression>(left);
        shared_ptr<Expression> right = cnd_expr->get_right();
        shared_ptr<NumberExpression> num_e =
            dynamic_pointer_cast<NumberExpression>(right);

        vector<string> cmp = {"cmpq", to_string(num_e->get_value()),
    ...
    } else if (shared_ptr<VariableExpression> var_expr =
                   dynamic_pointer_cast<VariableExpression>(anf)) {
      string var = var_expr->get_name();
...
int main() {
  ...
  /// string input = "(let ((sum 0)) (let ((i 0)) (begin (while (< i 5) (begin
