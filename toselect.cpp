...
public:
  ...
  to_select(const std::shared_ptr<Expression> &anf) {
    size_t counter = 0;
  ...
  anf_to_select(const std::shared_ptr<Expression> &anf, size_t &counter) {
    ...
    if (std::shared_ptr<LetExpression> anf_let =
      ...
      if (stack.find(let_var) == stack.end()) {
        ...
        std::vector<std::shared_ptr<Instruction>> body_instructions =
            anf_to_select(anf_let->get_body(), counter);
...
int main() {
  ...
  std::vector<std::shared_ptr<Instruction>> instructions =
      InstructionSelector::to_select(anf);
